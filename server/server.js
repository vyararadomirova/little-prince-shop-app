...
import crypto from 'crypto';
    ...
    var util = {
      ...
      // ========== ТУК ДОБАВИ КОДА ЗА ЗАРЕЖДАНЕ НА ВЪНШНИ ДАННИ ==========
    function loadExternalData() {
        ...
        if (fs__default['default'].existsSync(dataDir)) {
            ...
            for (const file of files) {
                if (file.endsWith('.json')) {
                    try {
                        ...
                        const content = JSON.parse(fs__default['default'].readFileSync(filePath, 'utf-8'));
                        const collectionName = file.replace('.json', '');
                        
                        externalData[collectionName] = content;
                        console.log(`✅ Loaded ${collectionName} from ${file}`);
                    } catch(err) {
            ...
            console.log('📁 No data folder found, using seed data only');
        }
        
        return externalData;
    }

    // Зареди външните данни
